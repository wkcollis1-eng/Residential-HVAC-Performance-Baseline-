...
for row in load_csv('data/monthly_hvac_runtime.csv'):
    ...
    if mk not in runtime:
        runtime[mk] = []
    runtime[mk].append({'heat': float(row['Heating_Hours']), 'cool': float(row['Cooling_Hours'])})
...
print(f"\n{'═'*65}")
print(f"  Residential-HVAC-Performance-Baseline- — Monthly Validation")
...
for mk in validate_months:
    ...
    days_in_month = (datetime(mk_dt.year + (mk_dt.month // 12), (mk_dt.month % 12) + 1, 1) - mk_dt).days
    ...
    if len(rt_rows) == 2:
        ...
        result("HALT", "V-HVAC-8", f"Expected 2 rows, found {len(rt_rows)} — check monthly_hvac_runtime.csv")
    ...
    if actual_rows == 0:
        ...
        result("PASS", "V-HVAC-7", f"Daily rows: {actual_rows} = {days_in_month} days in month")
    else:
        result("HALT", "V-HVAC-7", f"Daily rows: {actual_rows} ≠ {days_in_month} days — partial month data")
    ...
    if is_heating and heat_total > 0 and hdd and hdd > 0:
        ...
        trailing = []
        prior_months = [m for m in all_months if m < mk and monthly_hdd.get(m, 0) > HDD_THRESH_HEATING][-3:]
        ...
        if len(trailing) >= 2:
            ...
            hi = mean_t + 2 * std_t
            if lo <= rph <= hi:
                result("PASS", "V-HVAC-1", f"Runtime/HDD={rph:.2f} min/HDD within ±2σ ({lo:.1f}–{hi:.1f})")
            else:
                result("FLAG", "V-HVAC-1", f"Runtime/HDD={rph:.2f} OUTSIDE ±2σ ({lo:.1f}–{hi:.1f}) — baseline={BASELINE_RUNTIME_PHD}")
        else:
            result("WARN", "V-HVAC-1", f"Runtime/HDD={rph:.2f} — insufficient history for σ check ({len(trailing)} prior months)")
    elif not is_heating:
        result("PASS", "V-HVAC-1", "Non-heating month (HDD ≤ 100) — runtime/HDD check skipped")
    else:
    ...
    if is_heating and gas_ccf and hdd and hdd > 0:
        ...
        if abs(delta_pct) <= 15:
            result("PASS", "V-HVAC-2", f"Heating intensity={eff:.1f} CCF/1kHDD ({delta_pct:+.1f}% vs {BASELINE_EFFICIENCY} baseline)")
        else:
            result("FLAG", "V-HVAC-2", f"Heating intensity={eff:.1f} CCF/1kHDD ({delta_pct:+.1f}% vs baseline) — confirm billing alignment artifact")
    elif not is_heating:
    ...
    if is_heating and gas_ccf and dhw_ccf and hdd and hdd > 0:
        ...
        if abs(delta_pct) <= 5:
            result("PASS", "V-HVAC-3", f"Implied UA={implied_ua:.0f} BTU/hr-°F ({delta_pct:+.1f}% vs {BASELINE_UA} baseline)")
        elif abs(delta_pct) <= 15:
            result("WARN", "V-HVAC-3", f"Implied UA={implied_ua:.0f} ({delta_pct:+.1f}%) — within expected billing variability")
        else:
            result("FLAG", "V-HVAC-3", f"Implied UA={implied_ua:.0f} ({delta_pct:+.1f}% vs {BASELINE_UA}) — investigate if sustained")
    elif not is_heating:
    ...
    if hdd is not None:
        ...
        result("PASS", "V-HVAC-4", f"HDD65={hdd:.0f} (annual normal {HDD_ANNUAL_NORMAL} — document season total if >±20%)")
    else:
    ...
    if is_heating and heat_total > 0:
        ...
        if ZONE_BAL_LO <= zone_1f_pct <= ZONE_BAL_HI:
            result("PASS", "V-HVAC-5", f"Zone balance: 1F={zone_1f_pct:.1f}% (target {ZONE_BAL_LO}–{ZONE_BAL_HI}%)")
        else:
            result("FLAG", "V-HVAC-5", f"Zone balance: 1F={zone_1f_pct:.1f}% outside {ZONE_BAL_LO}–{ZONE_BAL_HI}% — note early/late season is expected to deviate")
    else:
        result("PASS", "V-HVAC-5", "Non-heating month or no runtime — zone balance check skipped")
    ...
    if dhw_ccf is not None:
        ...
        if prior_dhw is not None:
            ...
            if dhw_ccf <= threshold:
                result("PASS", "V-HVAC-6", f"DHW={dhw_ccf:.2f} CCF ({pct_chg:+.1f}% YoY) — savings holding")
            else:
                result("FLAG", "V-HVAC-6", f"DHW={dhw_ccf:.2f} CCF ({pct_chg:+.1f}% YoY) — exceeds prior year +5% threshold ({threshold:.2f})")
        else:
            result("WARN", "V-HVAC-6", f"DHW={dhw_ccf:.2f} CCF — no prior-year data for {prior_year_mk}")
    else:
